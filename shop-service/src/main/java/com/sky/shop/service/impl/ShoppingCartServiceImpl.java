...
import com.sky.gateway.utils.common.exception.ShoppingCartBusinessException;
import com.sky.shop.mapper.shoppingCartMapper;
    ...
    public void addCart(ShoppingCartDTO shoppingCartDTO){
        ...
        //若该数据不存在则进行添加
               ...
               if(shoppingCartDTO.getDishId()!=null){
                   DishVO dishById = dishClient.getById(shoppingCart.getDishId()).getData();
                   addShopCart(shoppingCart,dishById.getName(),dishById.getImage(),dishById.getPrice());
               }
               else{
               //Done：根据上步的操作就能判断是套餐，进行添加
                   SetmealVO byId = setmealClient.getById(shoppingCart.getSetmealId()).getData();
